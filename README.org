...
- There is no real syntax highlighting/indentation mode included.
  However the =js-mode= does a good enough job with most Haxe code that it's less urgent to create a specialized mode.
  The =use-package= block for =haxe-mode= (really =js-mode=) is good for things such as =yasnippet= saving your snippets under "haxe-mode".
